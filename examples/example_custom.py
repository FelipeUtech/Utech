...
- Carga lateral de 1000 kN sin momento
- Estratos definidos por módulo de Young (E)
"""
...
def main():
    print("=" * 80)
    print("ANÁLISIS DE PILOTE CON CARGA LATERAL - PARÁMETROS PERSONALIZADOS")
    print("=" * 80)
    print()
    ...
    pile = PileProperties.circular_pile(
        length=20.0,           # 20 metros de longitud total
    ...
    print("2. Estratos de Suelo (definidos por módulo E):")
    print("-" * 40)

    # Definir módulos de Young de cada estrato
    E1 = 10e6   # 10 MPa
    E2 = 20e6   # 20 MPa
    E3 = 80e6   # 80 MPa
    ...
    for i, (layer, E) in enumerate(zip(soil_layers, [E1, E2, E3]), 1):
        print(f"   Estrato {i}:")
        print(f"      Profundidad: {layer.depth_top} - {layer.depth_bottom} m")
        print(f"      Espesor: {layer.thickness} m")
        print(f"      Módulo E: {E/1e6:.1f} MPa")
        print(f"      k_h calculado: {layer.k_h/1e6:.2f} MN/m³")
    ...
    print("-" * 40)
    load = LoadCase(
        horizontal_load=1000e3,  # 1000 kN horizontal
    ...
    print("-" * 40)
    try:
        results.plot_results(save_path='pile_analysis_custom.png')
        print("   Gráficos guardados en: pile_analysis_custom.png")
